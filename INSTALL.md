...
- 근거 수준의 OpenCrab 조회가 필요하면 별도의 OpenCrab MCP 연결 필요
...
find /path/to/video-production-skills/skills -mindepth 2 -maxdepth 3 -name SKILL.md \
  -exec sh -c 'for f do cp -R "$(dirname "$f")" "$1"; done' sh .claude/skills {} +
```
...
2. **실제 예시**: [examples/brand-film-30s/](examples/brand-film-30s/) 결과물 보기
3. **커스터마이징**: 자신의 브랜드에 맞게 visual-bible 활용
